...
const KNOWN_LEAKED = ['Admin1234!', 'Manager1234!', 'Staff1234!', 'Client1234!']

function checkPassword(pw: string): string | null {
  if (pw.length < MIN_PASSWORD) return `Le mot de passe doit faire au moins ${MIN_PASSWORD} caractères.`
  if (KNOWN_LEAKED.includes(pw)) return 'Ce mot de passe figure dans la documentation publique du dépôt. Choisissez-en un autre.'
...
async function main() {
  ...
  if (existingAdmins > 0 && !RESET) {
    console.log(`ℹ️  ${existingAdmins} administrateur(s) actif(s) déjà présent(s) : rien à faire.`)
    console.log('   Mot de passe oublié ? Relancez avec --reset-password.')
    return
  }

  const email = (process.env.ADMIN_EMAIL ?? (interactive ? await ask('Email de l’administrateur : ') : '')).toLowerCase()
  if (!email || !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) {
    throw new Error('Email manquant ou invalide (ADMIN_EMAIL).')
  }

  const password = process.env.ADMIN_PASSWORD ?? (interactive ? await promptHidden('Mot de passe : ') : '')
  ...
  if (existing) {
    if (!RESET) {
      throw new Error(`Le compte ${email} existe déjà. Relancez avec --reset-password pour le réinitialiser.`)
    }
    ...
    console.log(`✅ Compte ${email} réinitialisé (rôle ADMIN, sessions révoquées).`)
    return
  }

  const name = (process.env.ADMIN_NAME ?? (interactive ? await ask('Nom affiché [Admin] : ') : '')) || 'Admin'
  await prisma.user.create({
    data: { name, email, password: hashed, role: 'ADMIN', isActive: true },
  })
  console.log(`✅ Administrateur créé : ${email}`)
  console.log('   Connectez-vous sur /admin/login, puis créez les autres comptes depuis Utilisateurs.')
}
